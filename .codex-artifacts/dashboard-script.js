        ...
        const body = rows.length
          ? rows
              .map((r, i) => {
                ...
                return `<tr><td class="em">${trunc(r.nome, 24)}</td><td class="r">${pctQtd.toFixed(1)}%</td><td class="r">${pctFat.toFixed(1)}%</td><td><div class="prg"><div class="prg-bg"><div class="prg-fill" style="width:${Math.min(100, Math.round(pctFat))}%;background:${PAL[i % PAL.length]}"></div></div><span class="prg-pct">${pctFat.toFixed(0)}%</span></div></td></tr>`;
              })
              .join("")
          : emptyRow(4);
        el.innerHTML = `<table><thead><tr><th>Destino</th><th class="r">% sob QT</th><th class="r">% sob R$</th><th>Participacao</th></tr></thead><tbody>${body}</tbody></table>`;
      }
  ...
  "jul", "ago", "set", "out", "nov", "dez",
      ...
      function ensureReviewLayouts() {
        ...
        if (frota && frota.dataset.reviewLayout !== "1") {
          ...
          frota.insertAdjacentHTML("afterbegin", `
            ...
            <div class="tc">
              <div class="tc-bar"><div><div class="tc-title">KPIs de manutencao</div><div class="tc-meta">Metas solicitadas na revisao</div></div></div>
              <div class="tc-wrap"><table><thead><tr><th>Categoria</th><th>KPI principal</th><th>Meta mensal</th><th>Limite anual</th></tr></thead><tbody>
                <tr><td>Preventiva programada</td><td>Manutencoes realizadas em ate 10 dias da solicitacao</td><td>&ge; 98%</td><td>&ge; 98%</td></tr>
                <tr><td>Preventiva por condicao</td><td>Problemas que evoluiram para pane</td><td>0</td><td>Maximo 2% dos casos</td></tr>
                <tr><td>Corretiva nao critica</td><td>Quantidade de ocorrencias</td><td>Maximo 5</td><td>Maximo 24</td></tr>
                <tr><td>Corretiva critica/emergencial</td><td>Pane que deixou veiculo indisponivel</td><td>Meta 0</td><td>Maximo 3</td></tr>
                <tr><td>Conservacao, conforto e imagem</td><td>Veiculos aprovados na inspecao</td><td>&ge; 95%</td><td>Media &ge; 95%</td></tr>
                <tr><td>Avaria ou sinistro preventavel</td><td>Ocorrencias atribuidas a conducao/operacao</td><td>Meta 0</td><td>Maximo 2</td></tr>
              </tbody></table></div>
            </div>
          `);
      ...
      function renderAll() {
        ...
        html("kRecebPctD", dvDisabled ? "" :
          `<span class="badge-kpi badge-lm"><span class="bk-label">REC</span><span class="bk-val">${brlS(recebidoProduzido)}</span><span class="bk-pct">recebido</span></span>` +
        ...
        // Faturamento por Cliente por Mês
        {
          ...
          if (cliEntries.length > 0) {
            ...
            document.getElementById("ind_fatcli").innerHTML = dvDisabled ? "" :
              `<span class="badge-kpi badge-lm"><span class="bk-label">1°</span><span class="bk-val">${brlS(top1Fat)}</span><span class="bk-pct">${top1Pct}%</span></span>` +
        ...
        if (top10v.length > 0) {
          ...
          document.getElementById("ind_topVol").innerHTML = dvDisabled ? "" :
            `<span class="badge-kpi badge-lm"><span class="bk-label">1°</span><span class="bk-val">${top10v[0][1].length}</span><span class="bk-pct">${t1vPct}%</span></span>` +
        ...
        if (top10f.length > 0) {
          ...
          document.getElementById("ind_topFat").innerHTML = dvDisabled ? "" :
            `<span class="badge-kpi badge-lm"><span class="bk-label">1°</span><span class="bk-val">${brlS(sumV(top10f[0][1]))}</span><span class="bk-pct">${t1fPct}%</span></span>` +
        ...
        if (!dvDisabled && mots.length > 0) {
          ...
          document.getElementById("ind_mot").innerHTML =
            `<span class="badge-kpi badge-lm"><span class="bk-label">1°</span><span class="bk-val">${topMot.count}</span><span class="bk-pct">${topMotPct}%</span></span>` +
